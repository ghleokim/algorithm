...
def add_db(db_dict, item, score):
    if db_dict.get(item) is None:
        db_dict[item] = {"all": {int(score): 0}}
    elif db_dict[item].get(int(score)) is None:
        db_dict[item]['all'][int(score)] = 0
    
    print('------')
    print(db_dict[item]['all'].keys())
    for s in db_dict[item]['all'].keys():
        if int(s) <= int(score):
            print(s, score, db_dict[item]['all'])
            db_dict[item]['all'][s] += 1
            print('    -      ', db_dict[item]['all'])
...
def solution(info, query):
    ...
    for info_item in info:
        ...
        if db_dict["all"].get(int(info_item[4])) is None:
            db_dict["all"][int(info_item[4])] = 0
        for s in db_dict["all"].keys():
            if int(s) <= int(info_item[4]):
                db_dict["all"][s] += 1
    ...
    for q_item in query:
        ...
        cur_result = 0
        for s in q_result.keys():
            if int(s) >= int(q_item[7]):
                cur_result = q_result[s]
                break
